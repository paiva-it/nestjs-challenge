import { stringifyUnknownVariable } from './stringify-unknown-variable.util';
...
describe('stringifyMongoQuery', () => {
  it('safely stringifies circular objects', () => {
    const query: any = { a: 1 };
    query.self = query;
    const result = stringifyUnknownVariable(query);
  ...
  it('stable sorting for logs (deterministic output)', () => {
    ...
    const q1 = buildQuery();
    const q2 = buildQuery();
    const r1 = stringifyUnknownVariable(q1);
    const r2 = stringifyUnknownVariable(q2);
    expect(r1).toBe(r2);
