...
export function buildCursorPaginationResponse<T extends { _id?: any }>(
  ...
  if (data.length > limit && pageItems.length > 0 && last._id) {
    response.nextCursor = String(last._id);
  ...
  if (deriveFromIds && pageItems.length > 0) {
    if (first._id) {
      response.startCursor = String(first._id);
    }
    if (last._id) {
      response.endCursor = String(last._id);
    }
  }
