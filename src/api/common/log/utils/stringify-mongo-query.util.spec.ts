import { stringifyMongoQuery } from './stringify-mongo-query.util';
...
describe('stringifyMongoQuery', () => {
  it('safely stringifies circular objects', () => {
    const query: any = { a: 1 };
    query.self = query;
    const result = stringifyMongoQuery(query);
  ...
  it('stable sorting for logs (deterministic output)', () => {
    ...
    const q1 = buildQuery();
    const q2 = buildQuery();
    const r1 = stringifyMongoQuery(q1);
    const r2 = stringifyMongoQuery(q2);
    expect(r1).toBe(r2);
